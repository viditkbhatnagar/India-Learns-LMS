...
## D-112 — Showcase: staff present marketing PDFs in-app (GridFS-seeded, view-only)
**Date:** 2026-07-12
**Why:** Client added 3 marketing PDFs (India Learns company profile + Fashion Design & Retail Management + Digital Fashion Entrepreneurship brochures, ~57/56/9 MB) and asked for "a specific section from where admin or teachers can open and showcase them in-app, in portal only." Audience confirmed **staff only** (Admin/Superadmin/Faculty); students excluded.

**Decisions:**
- **Storage = MongoDB GridFS, not S3.** Prod runs `STORAGE_PROVIDER=mongo` (bucket `il_files`) with no AWS creds wired; S3 is supported in code but not provisioned. GridFS works today with zero new infra and the auth-gated `/v1/files/:id` viewer URL is identical either way. If S3 is wired later, nothing in the feature changes.
- **Ingest via a seed script, not the HTTP upload route.** The upload route caps at 5 MB (`api/src/routes/files.ts`), so 55 MB files can't go through it. `scripts/seed-showcase.ts` writes bytes straight into GridFS via `new MongoStorageAdapter().upload(...)` — deliberately NOT `getIntegrations().storage`, which is the in-memory **stub** in dev (`STORAGE_PROVIDER=stub`) and wouldn't persist. `GET /v1/files/:id` resolves GridFS in its fallback chain regardless of provider, so seeded bytes serve in both dev(stub)+prod(mongo).
- **PDFs are gitignored (~120 MB).** Run the seed per-environment with the files present locally (dev DB, then prod Atlas) — same operational pattern as `seed:entrance` credentials. Not committed to git.
- **View-only now; not admin-manageable.** Self-serve upload of 55 MB brochures needs a streaming direct-to-GridFS path — deferred (YAGNI). Adding collateral = edit `DOCS` in the seed + re-run. The `ShowcaseDocument` model is intentionally extensible (slug/order/active/category) so management can be added later.
- **In-app viewer = authed blob → `<iframe>`.** `/v1/files/:id` is bearer-auth only (a plain `<iframe src>` would 401), so the viewer fetches the PDF via the axios client as a Blob → object URL → `<iframe>` with a fullscreen present mode. Dependency-free (no react-pdf) — respects the locked-deps rule.

**Shipped:** shared-types (`showcase` StorageFolder + `SHOWCASE_CATEGORIES` + `ShowcaseDocumentDto`), `ShowcaseDocument` model, `showcaseService`, `GET /v1/showcase(/:id)` staff-gated route, `seed:showcase`, web `showcaseApi` + `/showcase` page/viewer + nav (admin/superadmin/faculty). Tests: `showcaseService.test.ts` (unit) + `showcase.test.ts` (integration: admin/faculty 200, student 403, 401, 404). Smoke: `docs/smoke/showcase.md`.

**Byte-gating note:** PDF bytes are `requireAuth`-gated (any signed-in user who knows a `fileId`), consistent with all other files. `fileId`s are only returned to staff by `/v1/showcase`, so students have no path to them. Stricter gating would need a role-gated showcase file route.

### D-112 addendum — post-review hardening (2026-07-12)
An adversarial review (4 dimensions, each finding independently verified) surfaced two real high-severity issues, both fixed:
- **CSP blocked the blob `<iframe>` in prod.** Single-service prod serves the SPA from the API process (`app.ts` express.static of `web/dist`), so `helmet()`'s default CSP (`frame-src`→`default-src 'self'`) governs the page, and `'self'` does not cover `blob:`. Fixed: `app.ts` now sets `frame-src 'self' blob:` (spreads helmet defaults, overrides only frame-src). Without this the core "Present" flow renders a blank frame on the real deploy but works in dev (Vite has no CSP) — a bug local testing would miss.
- **PDF bytes weren't role-gated.** `/v1/files/:id` is `requireAuth`-only, so it didn't satisfy "students must never see it." Fixed: bytes now stream through the STAFF-gated `GET /v1/showcase/:id/file` (resolves the GridFS id server-side), and `fileId` was removed from `ShowcaseDocumentDto` entirely — the client only ever holds the ShowcaseDocument id.
Also fixed (medium/low): seed reordered to upload→upsert→delete-old (no orphan/dangle on mid-run failure); added `gridfsFileExists` (cheap existence check, no unconsumed download stream); removed redundant single-field indexes + unused `deleteShowcaseDocumentBySlug`; viewer got a focus trap + initial/restore focus, an independent object URL for "Open in new tab" with a pop-up-block download fallback.
**Test-env gotcha:** real GridFS byte round-trips can't run under this repo's `node_modules` (mongoose's bundled bson 6.x vs the top-level `mongodb`'s bson → `BSONVersionError`); the whole suite only uses stub storage for this reason. So `/:id/file`'s staff happy-path is covered by the smoke doc; the student-403 (enforced at middleware, no GridFS) is the automated guarantee.
...
## D-115 — Forgiving input on all public/staff forms + end-to-end user guides (2026-07-22)

**Why:** Staff/prospects kept hitting the generic *"Request failed validation"* wall on two classes of input: (a) a bare 10-digit phone (colleague Athira, then the visitor/admissions/apply forms), and (b) a program **slug** typed with a space (`retail management-diploma`) on the *Create program* form.

**Phone (fix commit `363a139`):** extended the shared `phoneE164Schema` (utils/phone) — already on faculty/user create — to the remaining route input schemas: public visitor register, admin visitor-leads create/update, admissions applicant signup, add-referee. Apply-flow draft phones are stored raw (`SaveDraftBody.payload` = `z.unknown()`), so emergency/parent contact phones are normalized at **conversion** (`copyPersonalDetailsFromDraft`) before hitting the strict User model (skip if un-normalizable, non-fatal). Web: shared `normalizePhoneLoose` + `PHONE_HINT`; removed native `pattern="^\+..."` blocks + "(with country code)" copy from apply forms. NOTE: `userService.update` emergency/parent were left trusting the route's `ContactRefBody` (already normalizes) — the codebase pattern is normalize-at-route, trust-in-service; don't re-normalize in services.

**Slug (fix commit `0a38158`):** new `utils/slug.ts` `slugify()` + `slugSchema` (zod transform: lowercase, strip accents, non-alnum→single hyphen, trim). Programs create/update use it instead of `^[a-z0-9-]+$`. Web `AdminPrograms`: Slug auto-fills from Name (until hand-edited) + slugifies on blur; `web/lib/slug.ts` mirrors it. **Pattern:** forgiving zod `.transform()` at the route boundary + a clear custom error, mirroring `phoneE164Schema`.

**Guides (commit `19cc25b`):** `docs/guides/India-Learns-{Admin,Faculty}-Guide.md` (+ PDF, also copied to ~/Downloads) — plain-language, button-by-button, built from verified UI labels then adversarially verified (3 agents, 17 findings applied). **Key product truths the guides surfaced (worth improving in-app):** (1) admins are **read-only in a course** ("oversight mode") — content upload + publish are the assigned **teaching faculty's** job; courses come from **curriculum import**, not a create form. (2) Ticking *Enrol* on Create login **silently no-ops** if the program has no courses yet (best-effort swallow) — no user warning. (3) Timetable builder takes raw Course/Faculty **IDs** + minutes-from-midnight (not user-friendly). (4) Faculty course click → tabbed Course Shell; announcements/assignments live behind *Announcements → Open legacy composer*.

## D-116 — Create-login enrol no longer fails silently (2026-07-22)
`createUserAccount` now returns `enrolmentWarning: string | null`; when enrol was requested but produced 0 course access (program has no courses → `enrolStudentInProgram` throws), the login is still created (best-effort unchanged) but the reason is surfaced through `POST /v1/users` and shown as an amber notice under the Create-login success box. Commit `8bfdc5e`. **Live-verified on prod** (temp-superadmin script, cleaned up): slug forgiveness, phone forgiveness, and the enrol-warning all confirmed working. Push-to-main → Render auto-deploy; verify with a self-polling script keyed on the new behaviour (see [[git-identity]] for the push/account dance).
...
## D-119 — Add a lesson manually from the Content tab (2026-07-22)
Lessons could only arrive via curriculum import. Added `POST /v1/courses/:courseId/sessions` (`createSession` in sessionService) — appends after the module's last lesson (avoids the unique `(moduleId, number)` index), gated by `assertFacultyCanWriteCourse` (assigned faculty only; admins in oversight get 403). Manual lessons carry **`sourceLessonId=null` + `synthesized=false`**, so a later curriculum re-import (persister only wipes generator-sourced/synthesized children) leaves them intact. New audit action `session.created` (added to AUDIT_ACTIONS in shared-types — rebuild shared-types after enum edits). Web: `sessionsApi.create` + an `AddLessonForm` per module in `ContentTab` (hidden in oversight mode). Faculty guide §5 updated. Tests in `sessions.lifecycle.test.ts` (append/no-source-id/oversight-403/student-403). Commit `b39a89b`. Full suite: 673 pass; the lone failure (`assignmentSubmissions.twoStep`) is a parallel-load isolation flake (passes alone).
...
## D-124 — Generator fixed at source; both fashion courses rebuilt from it (2026-07-25)

**Generator side (separate repo/app, done by its own agent):** root cause was NOT byte-identical duplicates — it was (a) `lessonPlanService.ts` deriving a lesson QUOTA from contact hours (`round(contactHours*60/90)`) and asking the LLM for exactly that many, padding with synthetic filler ("Practical Evidence Build", "Peer Review and Revision Workshop", "Integrated studio block — …"), and (b) lesson titles being the MLO statement rather than the AI title, so lessons sharing an MLO got identical titles. Filler is always 90 min while genuine lessons carry the module's full duration — filtering on that reproduced the SME's 214 exactly. Digital Fashion wasn't missing step10-13; a step-9 re-run had rewound `currentStep` to 9, which is what blocked our import. Fix committed on branch `fix/step10-filler-lessons` (6120272) — **NOT pushed** (would auto-deploy), so the defect can still recur.

**LMS side:** the doc-ingest had detached both courses (`sourceWorkflowId = null`), so a generator re-import would duplicate them — and for Fashion it actually returned **409 SLUG_EXISTS** (unique `(programId, slug)` is NOT partial on `deletedAt`). Added **adoption**: `runImport`/`persistImport` accept an optional `courseId` (`adoptCourseId`) to point an existing course at a workflow and rebuild in place, keeping the course `_id` (enrolments, faculty roster, links). Guards: PROGRAM_MISMATCH, COURSE_TRACKS_OTHER_WORKFLOW, COURSE_IN_USE; adoption forces replace and soft-deletes EVERY live child (doc-sourced children have no `source*Id` and would otherwise survive alongside the new ones). Commit `4caf723`.

**Result (verified live):** both courses = 214 teaching lessons with the SME's exact per-module split + 9 auto-generated (8 module assessments + final exam) = 223 sessions; slides 209 / 114; assignments 25 each; same course ids; Athira still assigned; previous doc lessons archived.

**Open (generator, not ours to fix):** PPT coverage gaps (Fashion 5 of 214 lessons with no deck, **Digital 100 of 214**) — closing needs a real step11 run (~314 LLM generations). The Styling programme (`6a0406fc…`) has the same title-collision defect, untouched. Generator prod has **no Auth0** — `authBypass` mints an anonymous ADMINISTRATOR, so every POST/PUT there is open to the internet.

## D-125 — Generator regression + permanent fix; LMS verified in sync (2026-07-28)

**What happened:** after the first generator fix, Fashion (`69c4fa84…`) was regenerated on 28 Jul 04:24 and went back to **424** lessons (per-module 54/60/70/67/38/43/42/50 = exactly `round(contactHours*60/90)` — the quota bug), `currentStep` rewound 13→10, 32 dangling decks. Cause: the amber **"Regenerate" button on a module card in Step 10** (`Step10View.tsx:1580` → `POST /:id/step10/module/:moduleId/regenerate`), which chains across all modules via `queueAllRemainingModules`. The first fix wouldn't have prevented it — `regenerateStep10Modules` **deletes the plan before** the count is read, so "reuse existing count" read undefined. Now fixed by recording `step10.plannedLessonCounts` before deletion (generator `main` @ `36a24b9` + `6120272`, deployed).

**Verified by me against the live API (28 Jul):** both workflows 214 with the SME's exact per-module splits, unique lessonIds, **0 dangling decks**, 8 packs each correctly keyed, step13 present, currentStep 13. The SME's Word export (`GET /api/v3/workflow/:id/export/word/step/10`, content-addressed S3 cache, no auth) downloads clean at 214 — I fetched and parsed the real file with our own `docxExtract` + `parseLessonPlan`. **LMS ↔ generator lessonIds match 1:1 (0 drift), so no re-import was needed.**

**Gotcha for future checks:** step4 modules key on **`id`** (values like `mod-imported-39d60b4c1a3a`, `mod2`), NOT `moduleId`; `step12` packs reference that same `id` in their `moduleId`. Our transformer keys on `m.id` (transformer.ts:268/705) — checking against `m.moduleId` produces a false "all packs orphaned" alarm.

**Still open on the generator (not ours):** production has **no auth** — any unauthenticated POST can trigger a regeneration (and step10 routes write no audit entry, so the 04:24 trigger is untraceable); PPT coverage Fashion 209/214, **Digital 114/214**; Styling `6a0406fc…` still has the title-collision defect. Tell staff: **never press the amber Regenerate button on a Step 10 module card** (recoverable via Version history if they do).

## D-126 — Plain "Module 1/2/3" naming instead of generator codes (2026-07-28)
SME: the generator's codes (MOD101, MOD302 — level-encoded, not sequential) read as an error to anyone outside the generator. LMS side (commit `19a6ad5`): Content-tab module header now reads **"Module 1"** (was "M1"); the raw `MOD101` line removed from the module overview panel (the `code` field is still stored, just not shown); auto-generated assessment sessions retitled from `"${code} — Assessment"` to `"${module title} — Assessment"` — which also fixed Digital Fashion, whose modules have an **empty-string** `code` so the old `?? ` fallback produced a bare `"— Assessment"`. Test asserts no synthesized title contains a MOD code. Both fashion courses re-synced in place (adopt path) → 214 lessons, 9 auto sessions, 0 bad titles, faculty preserved. Generator-side (its Word export headings + Step-10 UI) was handed to that repo as a **display-only** change — its `step4.modules[].id/.code`, `step12` moduleIds and `step11` lessonIds must not change or referential integrity + our import break. Note: our `parseLessonPlan` already accepts `Module N:` headings, so the new export format needs no LMS change.

**Ops lesson:** when verifying a change that only shows up in *behaviour*, don't poll for "API responds" (the old build answers too) — poll for the new behaviour itself and retry. A resync raced the Render deploy and silently did nothing; the retry-until-verified version caught it.

## D-127 — Delete a program from the UI (2026-07-28)
`DELETE /v1/programs/:id` already existed (admin-gated, soft-delete `deletedAt` + `isActive=false`, audited) but nothing in the web app called it. Added `programsApi.remove()` + a per-row **Delete → Confirm delete** action on Admin → Add & View Programs (`ProgramRow` component), surfacing the server's 409 reason inline. **Guard strengthened:** `deleteProgram` only counted courses, so a program with **batches / enrolments / students / fee structures** could be deleted and strand them (all six collections carry `programId`: course, batch, enrollment, user, feeStructure, announcement). Now counts five of them (announcements deliberately ignored — harmless if orphaned) and refuses with the blockers named ("still has 2 batches, 5 students"). Commit `de34f55`; suite 687 pass (2 red = the known MongoMemoryServer flake, 0 assertion errors, pass in isolation).
