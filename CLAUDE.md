...
## 0. Current repo state (read before touching anything)

This repo is **pre-implementation**. As of the last update there is no application code — only the spec pack and approved design mockups.

**What exists:**
- [claude-code-docs/](claude-code-docs/) — the source-of-truth doc pack: [01_BRD.md](claude-code-docs/01_BRD.md), [02_PRD.md](claude-code-docs/02_PRD.md), [03_TRD.md](claude-code-docs/03_TRD.md), [04_UI_UX_Spec.md](claude-code-docs/04_UI_UX_Spec.md), [05_Deployment_Runbook.md](claude-code-docs/05_Deployment_Runbook.md).
- [webapp/](webapp/) — approved React/JSX mockups + CSS (`screens-staff.jsx`, `screens-student.jsx`, `screens-student2.jsx`, `screens-extras.jsx`, `components.jsx`, `styles.css`, `responsive.css`). **Port these into `web/`; do not redesign.**
- [mobile/](mobile/) — approved mobile PWA mockups (`screens-mobile.jsx`, `components.jsx`, `ios-frame.jsx`, `styles.css`).
- [PROMPTS.md](PROMPTS.md) — the milestone-by-milestone prompt script driving the build.

**What does NOT exist yet** (and will be created during M1 per §4): the `api/`, `web/`, `packages/shared-types/`, `memory/`, and `TASKS.md` scaffolding described throughout this file. If a future session references them and they're missing, you're at M1 and need to create them.
...
Once M1 scaffolds the monorepo (npm workspaces, root `package.json`), the canonical commands will be:

| Purpose | Command |
|---|---|
| Install everything | `npm install` (from repo root) |
| Run API dev server | `npm run dev -w api` |
| Run web dev server | `npm run dev -w web` |
| Run both concurrently | `npm run dev` (root script wired in M1) |
| Lint | `npm run lint` (root), or `npm run lint -w api` / `-w web` |
| Typecheck | `npm run typecheck` (root) |
| Test (all) | `npm test` |
| Test a single file | `npm test -- path/to/file.test.ts` (Vitest) |
| Test a single name | `npm test -- -t "matching name"` |
| Coverage | `npm test -- --coverage` (must keep `services/` ≥ 70% per §5) |
| Seed super-admin | `npm run seed:superadmin -w api` (from M1 step 4) |
| Build for prod | `npm run build` (root) |

**When scaffolding in M1, wire these scripts first** so every later milestone can rely on them. If a command here doesn't exist yet, you're the session that needs to add it — don't invent alternatives.
